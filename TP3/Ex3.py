...
class Pokemon:
    ...
    def takeDamage(self, amount: int):
        ...
        if type(amount) == int:
            if (self.pv < amount):
                ...
                self.pv -= amount
            print(f"{self.nom} a pris\033[31m {amount}\033[0m dágâts! {self}")
    ...
    def attaquer(self, other: Pokemon):
        ...
        if isinstance(other, Pokemon):
            print(f"{self.nom} attaque {other.nom} !")
    ...
    def combattre(self, autre: Pokemon):
        ...
        if isinstance(autre, Pokemon):
            ...
            while (not self.estKo() and not autre.estKo()):
                ...
                autre.attaquer(self)
            if (autre.estKo()):
                print(f"{autre.nom} est K.O. !")
                return (self, i)
            print(f"{self.nom} est K.O. !")
            return (autre, i)
    ...
    def __str__(self):
        return f"{self.nom} : [{self.pv}/{self.max_pv} PV(s); {self.atk} ATK]"
...
class PokemonFeu(Pokemon):
    ...
    @property
    def nom(self) -> str:
        """Overload de la property nom qui permet un affichage en couleur"""
        return f"\033[38;2;255;165;0m{self._nom}\033[0m"
    
    @nom.setter
    def nom(self, val) -> None :
        self._nom = str(val)
...
class PokemonEau(Pokemon):
    ...
    def nom(self) -> str :
        """Overload de la property nom qui permet un affichage en couleur"""
        return f"\033[34m{self._nom}\033[0m"
...
class PokemonPlante(Pokemon):
    ...
    def nom(self) -> str:
        """Overload de la property nom qui permet un affichage en couleur"""
        return f"\033[32m{self._nom}\033[0m"
...
def createPokemon():
    ...
    type_p = control.secureAskType(str, "Quel type de pokémon voulez-vous ? ", lambda x: x.lower().strip() in ["plante", "normal", "feu", "eau"])
    hp = control.secureAskType(int, "Entrez les points de vie, de type entier : ")
    atk = control.secureAskType(int, "Entrez les points d'attaque, de type entier : ")
    nom = input("Entrez le nom du Pokémon : ")
    attr = (nom.capitalize(), hp, atk)

    match type_p.lower().strip():
        case "plante":
...
def pickPokemon(list_pokemons: list | Pokemon) -> Pokemon:
    """Permet de récupérer un pokémon de la liste. Rendu plus ou moins obselète par conntrol.secureDisplayAndPick"""
    return control.secureDisplayAndPick(list_pokemons, "Entrez un identifieur de Pokémon : ", 1)
...
def combat(list_pokemons: list | Pokemon) -> None:
    ...
    if (pokemon1 != pokemon2):
        ...
        print("======================================")
        print(f"{result[0].nom} gagne en {result[1]} tour(s)")
...
def importer(pokedex_dict: dict) -> list:
    """Permet d'importer le dictionnaire pokédex."""
