...
def createPoupee() -> PoupeeRusse:
    ...
    nom = control.secureAskType(str, "Quel est le nom de la poupée : ", lambda x : len(x) > 0)
    taille = control.secureAskType(int, "Quelle est sa taille (entier) : ", lambda x : x > 0)
...
def insererPoupee(list_poupees : list[PoupeeRusse]) -> None:
    ...
    p1 = control.secureDisplayAndPick(list_poupees, "Entrez l'identificarteur de la poupée à placer dans l'autre : ", 1)
    p2 = control.secureDisplayAndPick(list_poupees, "Entrez l'identificarteur de la poupée qui va recevoir : ", 1)
...
def main() :
    ...
    try:
        ...
        while continuer :
            ...
            option = control.secureAskType(int, "Quelle option choissez-vous ? ", in_list)
            ...
            match option :
                ...
                case 2 :
                    if len(list_poupees) > 0 :
                        control.secureDisplayAndPick(*arg_display_and_pick).ouvrir()
                    else:
                        print("Vous n'avez aucune poupée à ouvrir.")
                case 3 :
                    if len(list_poupees) > 0 :
                        control.secureDisplayAndPick(*arg_display_and_pick).fermer()
                    else:
                ...
                case 5:
                    if len(list_poupees) >= 2 :
                        sortirPoupee(control.secureDisplayAndPick(*arg_display_and_pick))
                    else:
                        print("Vous n'avez pas assez de poupées.")
                case 6 :
                    if len(list_poupees) == 0:
...
• est_ouverte (booléen)
• dans (contenue dans une autre poupée)
• contient (contient une autre poupée).

• ouvrir(self) : ouvre la poupée si elle n’est pas déjà ouverte et si elle ne se trouve pas
à l’intérieur d’une autre poupée.
• fermer(self) : ferme la poupée si elle n’est pas déjà fermée et si elle ne se trouve pas
à l’intérieur d’une autre poupée.
• placer_dans(self, p) : place la poupée courante dans la poupée p si elle n’est pas
déjà dans une autre et si la poupée p ne contient aucune autre poupée et si la poupée
courante est fermée et la poupée p est ouverte et plus grande.
• sortir_de(self) : sort la poupée courante d’une autre poupée si la contenante est
bien ouverte.
• __str__(self) : qui retourne les informations de la poupée sous forme de chaîne. At‑
tention à bien retourner le nom des poupées contenues / contenant."""